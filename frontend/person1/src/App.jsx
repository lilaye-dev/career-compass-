...
export default function App() {
  ...
  const [screen, setScreen] = useState("form"); // form | email | success
  ...
  const handleGenerate = async () => {
    ...
    setLoading(true);
    setError(null);
    setProfile({ year, branch, goal, skills });
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-6" style={{background: "linear-gradient(135deg, #0f0c29, #302b63, #24243e)"}}>
        <div className="w-full max-w-md">
          <div className="rounded-3xl p-8 text-center" style={{background:"rgba(255,255,255,0.05)", backdropFilter:"blur(20px)", border:"1px solid rgba(255,255,255,0.1)"}}>
            <div className="text-5xl mb-4">🧭</div>
            <h2 className="text-2xl font-bold text-white mb-2">Your roadmap is ready!</h2>
            <p className="text-indigo-300 text-sm mb-8">Enter your email to receive your 12-week plan and weekly Monday reminders.</p>
            ...
            <div className="flex flex-col gap-4">
              <input
                ...
                className="w-full rounded-xl px-4 py-3 text-sm text-white outline-none focus:ring-2 focus:ring-indigo-500 placeholder-indigo-400"
              ...
              <button
                ...
                className="w-full py-4 rounded-xl font-bold text-white text-sm tracking-wide transition-all duration-200 hover:scale-[1.02]"
      ...
      <div className="min-h-screen flex items-center justify-center px-6" style={{background: "linear-gradient(135deg, #0f0c29, #302b63, #24243e)"}}>
        <div className="w-full max-w-md">
          <div className="rounded-3xl p-8 text-center" style={{background:"rgba(255,255,255,0.05)", backdropFilter:"blur(20px)", border:"1px solid rgba(255,255,255,0.1)"}}>
            ...
            <p className="text-indigo-300 text-sm mb-2">Your 12-week roadmap has been sent to:</p>
            <p className="text-white font-semibold mb-6">{email}</p>
            <p className="text-indigo-400 text-xs mb-8">You'll receive weekly Monday reminders with your next week's tasks.</p>
            <button
              onClick={() => setScreen("roadmap")}
    ...
    return (
      <div className="min-h-screen flex" style={{background: "linear-gradient(135deg, #0f0c29, #302b63, #24243e)"}}>
        <div className="hidden lg:flex flex-col justify-center px-16 w-1/2">
          ...
          <div className="flex flex-col gap-4">
            {["Personalized to your branch, year & goal", "Week-by-week actionable tasks", "Free resources only — zero cost", "Weekly email reminders every Monday"].map((item, i) => (
            ...
            <div className="rounded-3xl p-8" style={{background:"rgba(255,255,255,0.05)", backdropFilter:"blur(20px)", border:"1px solid rgba(255,255,255,0.1)"}}>
              ...
              <div className="flex flex-col gap-4">
                ...
                <div>
                  ...
                  <textarea id="skills" rows={3} placeholder="Enter what skills you have and what you want to learn further, or the career you want to pursue..." className="w-full rounded-xl px-4 py-3 text-sm text-white resize-none outline-none focus:ring-2 focus:ring-indigo-500 placeholder-indigo-400" style={{background:"rgba(255,255,255,0.08)", border:"1px solid rgba(255,255,255,0.15)"}}></textarea>
                </div>
  ...
  return <RoadmapView roadmapData={roadmapData} onBack={() => { setRoadmapData(null); setScreen("form"); }} />;
}